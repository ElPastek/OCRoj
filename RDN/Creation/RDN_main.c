...
static size_t nb_out = c->nb_out;
static size_t nb_ins = c->nb_ins;
static size_t nb_hne = c->nb_hne;
//size_t nb_col = c->nb_col;
static size_t nb_tot = c->nb_tot;
//end declare
...
void train(s_network *c, size_t nb_training_set, int ***inputs)
{
		...
		for (size_t training_set = 0; training_set < nb_training_set; training_set++)
		{
			for (size_t out = 0 ; out < nb_out ; out++)
			{
				...
				for (; j > (nb_ins - 1) ; j--) //hidden layers
				{
					...
					size_t next_max = next + next_len - 1;
				...
				verif_bool = verif_bool && (waited == real);
	...
	} while (!verif_bool && epoch < 10); //) );     //!(verif(r_results, nb_out))
